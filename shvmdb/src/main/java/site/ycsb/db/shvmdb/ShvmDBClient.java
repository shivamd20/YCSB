...
public class ShvmDBClient extends DB {
  ...
  private static String SHARED_TABLE_NAME;
  private static boolean IS_AUTO_GENERATED;
  private static final Object LOCK = new Object();

  private String endpoint;
  private String tableName;
  private boolean autoGeneratedTable;
  ...
  public void init() throws DBException {
    ...
    synchronized(LOCK) {
      if (SHARED_TABLE_NAME == null) {
        String userTable = getProperties().getProperty("shvmdb.table");
        if (userTable != null && !userTable.isBlank()) {
            SHARED_TABLE_NAME = userTable;
            IS_AUTO_GENERATED = false;
        } else {
            SHARED_TABLE_NAME = "bench_" + System.currentTimeMillis() + "_" + (int)(Math.random() * 1000);
            IS_AUTO_GENERATED = true;
        }
      }
    }
    
    this.tableName = SHARED_TABLE_NAME;
    this.autoGeneratedTable = IS_AUTO_GENERATED;
    
    // Always attempt to create the table. If it exists, shvm-db returns error which we can log.
    try {
        createTable(tableName);
    } catch (DBException e) {
      LOGGER.severe("Table " + tableName + " creation failed: " + e.getMessage());
      if (debug) {
        e.printStackTrace();
      }
      // Depending on requirements, we might want to throw here or proceed.
      // Proceeding might be dangerous if table truly failed to create.
    }
    ...
    LOGGER.info("ShvmDBClient initialized: endpoint=" + endpoint + ", table=" + tableName + 
                ", autoGenerated=" + autoGeneratedTable);
  ...
  public Status read(String table, String key, Set<String> fields, Map<String, ByteIterator> result) {
    try {
      ...
      requestBody.addProperty("TableName", this.tableName);
  ...
  private void createTable(String name) throws DBException {
    ...
    try {
      ...
      if (e.getMessage() != null && e.getMessage().contains("already exists")) {
        LOGGER.info("Table " + name + " already exists, skipping creation.");
        return;
      }
  ...
  public void cleanup() throws DBException {
     if (this.autoGeneratedTable && this.tableName != null) {
